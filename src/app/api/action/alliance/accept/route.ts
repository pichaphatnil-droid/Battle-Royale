...
export async function POST(request: Request) {
  try {
    ...
    const { data: invite } = await supabase.from('alliance_invites').select('*').eq('id', invite_id).single()
    ...
    const { data: joiningPlayer } = await supabase.from('players').select('*').eq('id', joiningPlayerId).single()
    ...
    if (invite.alliance_id) {
      const { data: alliance } = await supabase.from('alliances').select('*').eq('id', invite.alliance_id).single()
      ...
      await supabase.from('alliances').update({ members: [...members, joiningPlayerId] }).eq('id', alliance.id)
      ...
      const { data: fromPlayer } = await supabase.from('players').select('*').eq('id', invite.from_player_id).single()
      if (!fromPlayer) return NextResponse.json({ error: 'ไม่พบผู้ชวน' }, { status: 400 })
      const { data: newAlliance, error: alErr } = await supabase.from('alliances').insert({
        game_id,
      ...
      await supabase.from('players').update({ alliance_id: allianceId }).eq('id', fromPlayer.id)
      msg = `รวมกลุ่มกับ ${fromPlayer.name} แล้ว`
    }

    await supabase.from('players').update({ alliance_id: allianceId }).eq('id', joiningPlayerId)
