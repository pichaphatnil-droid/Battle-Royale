...
export async function POST(request: Request) {
  try {
    ...
    // --- เปลี่ยนเงื่อนไขการตรวจสอบ: รองรับ stat_bonus และ ap_bonus (ใช้กับอุปกรณ์/หนังสือ/เปลนอน) ---
    const hasEffect = effect && (effect.hp || effect.hunger || effect.thirst || effect.removes_moodle || effect.ap_bonus || effect.stat_bonus || effect.int_bonus)
    if (!hasEffect) return NextResponse.json({ error: 'ไอเทมนี้ไม่สามารถกดใช้งานได้' }, { status: 400 })

    // --- ตรวจสอบเงื่อนไข HP/AP เต็มแยกตามประเภทไอเทม ---
    if (effect.hp && player.hp >= player.max_hp) return NextResponse.json({ error: 'HP เต็มแล้ว ไม่จำเป็นต้องใช้' }, { status: 400 })
    if (effect.ap_bonus && player.ap >= 600) return NextResponse.json({ error: 'AP เต็ม 600 แล้ว ไม่สามารถพักผ่อนเพิ่มได้' }, { status: 400 })
    ...
    // หัก AP_Cost ของการใช้งานไอเทม (ถ้ามี)
    ...
    // --- จัดการ AP Bonus (เช่น เปลนอน เพิ่ม 1000) ---
    if ((effect.ap_bonus ?? 0) > 0) {
      const currentApCalculated = calcAP(player.ap, player.ap_updated_at)
      // หัก ap_cost ออกก่อน แล้วค่อยบวกโบนัส แล้วล็อคที่ 600
      updates.ap = Math.min(Math.max(0, currentApCalculated - apCost) + (effect.ap_bonus as number), 600)
      updates.ap_updated_at = new Date().toISOString()
      msgs.push(`ฟื้นฟู AP +${effect.ap_bonus}`)
    }

    // --- จัดการ Stat Bonus (เช่น หนังสือวิทยาศาสตร์ เพิ่ม INT) ---
    if (effect.stat_bonus) {
      for (const [stat, value] of Object.entries(effect.stat_bonus)) {
        if (player[stat] !== undefined) {
          // ล็อคสเตตัสสูงสุดที่ 15
          const oldVal = player[stat]
          const newVal = Math.min((oldVal ?? 0) + (value as number), 15)
          updates[stat] = newVal
          msgs.push(`${stat.toUpperCase()} ${oldVal}→${newVal}`)
        }
      }
    }
    
    // Fallback ของเก่าเผื่อใช้ int_bonus เดี่ยวๆ
    if (effect.int_bonus && !effect.stat_bonus) {
      updates.int = Math.min((player.int ?? 0) + (effect.int_bonus as number), 15)
    ...
    if (sideEffectTriggered && !newMoodles.some((m: any) => m.id === effect.side_effect_moodle)) {
      ...
      msgs.push(effect.side_effect_cancels_main ? `เกิดผลข้างเคียง! ติด ${effect.side_effect_moodle}` : `ติด ${effect.side_effect_moodle}!`)
    }

    const { newMoodles: finalMoodles } = await applyStatThresholdMoodles(
      supabase, 
      { ...player, moodles: newMoodles }, 
      { hunger: updates.hunger, thirst: updates.thirst, hp: updates.hp, maxHp: player.max_hp }, 
      moodleDefs ?? undefined
    )
    updates.moodles = finalMoodles

    // หักไอเทมออกจากกระเป๋า
    ...
    return NextResponse.json({ 
      ok: true, 
      msg: `ใช้งาน ${itemDef.name} สำเร็จ: ${msgs.join(', ')}`, 
      hp: updates.hp, 
